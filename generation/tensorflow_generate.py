...
class basic_rnn_model():
	def __init__(self,num_layers,lr,rnn_block,embed_matrix1,end_token,num_units,fc1,fc2,vocab_size,device):
		...
		def decode(helper, scope,batch_size,reuse=None):
			with tf.variable_scope(scope, reuse=reuse):
				...
				outputs = tf.contrib.seq2seq.dynamic_decode(
				    decoder=decoder,impute_finished=True, output_time_major=False
				     )
				return outputs[0]
...
def run(data,numepochs,save_model_after_n_epochs,num_layers,lr,rnn_block,num_units,fc1,fc2,vocab_size,model_save_dir,dictionary,num_training_examples,device,keep_prob=0.5,minibatch_size=32,testduringtrain=True,restore=False):
	...
	start_epoch_num=0
	if restore==True:
		start_epoch_num=json['num_epochs_over']
		latest_model_path=json['model_path']

	model=basic_rnn_model(num_layers,lr,rnn_block,embed_matrix1,endtoken,num_units,fc1,fc2,vocab_size,device)
	...
	with tf.Session(graph=model.graph) as sess:
		...
		for k in range(start_epoch_num,numepochs):
			l=0
			for i in tqdm(range(int(num_training_examples/minibatch_size))):
				cur_ip=data[i*minibatch_size:i*minibatch_size+minibatch_size]
			...
			if k%save_model_after_n_epochs==0:
				try:
					...
					os.mkdir(model_save_dir+'/genation_model_'+str(k)+'/gen_model')
				except:
					pass
				saver.save(sess,model_save_dir+'/genation_model_'+str(k)+'/gen_model')
			json_data['model_path']=model_save_dir+'/genation_model_'+str(k)+'/gen_model'
			json_data['num_epochs_over']=k
			json.dump(json_data,meta_file)